...
func applyDocumentationDefaults(spec *ActionSpec) {
	if spec.Category == "" {
		...
		case "read_files", "list_files", "search_code", "inspect_file", "create_directory":
	...
	if len(spec.ParameterNotes) == 0 {
		...
		case "read_files":
			spec.ParameterNotes = map[string]string{"files": "Array of {path, start_line, end_line}; shorthand string paths are accepted."}
		case "search_code":
		...
		case "run_command":
			spec.ParameterNotes = map[string]string{"command": "Executable only, or a simple whitespace-separated command for compatibility; no shell operators.", "args": "Separate argv-style arguments.", "working_directory": "Relative directory from the connected root.", "timeout_seconds": "Bounded timeout; use the smallest useful value."}
		case "run_commands":
